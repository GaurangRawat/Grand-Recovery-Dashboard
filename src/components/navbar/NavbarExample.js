...
export default function AuthNavbar(props) {
  ...
  routes.forEach((route) => {
    if (route.items) {
      const authLink = route.items.find((link) => link.name === "Authentication");
      if (authLink) {
        authObject = authLink;
      }
    }
  ...
  routes.forEach((route) => {
    if (route.items) {
      const applicationsLink = route.items.find((link) => link.name === "Applications");
      if (applicationsLink) {
        applicationsObject = applicationsLink;
      }
    }
  });
  ...
  routes.forEach((route) => {
    if (route.items) {
      ecommerceObject = route.items.find((link) => link.name === "Ecommerce") || ecommerceObject;
    }
  });
